...
def load_data():
    data_dir = Path(__file__).parent.parent / "output" / "20260520"
...
def check_all_stocks(data):
    for result in data.get("results", []):
        ...
        if forum_posts:
            print("\n前15个帖子:")
            for i, post in enumerate(forum_posts[:15], 1):
                title = post.get("title", "")
                url = post.get("url", "")
                source = post.get("source", "")
                print(f"\n{i:2d}. [{source}] {title[:70]}")
                print(f"    {url}")
